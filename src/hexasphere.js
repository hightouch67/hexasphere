...
class Point {
    ...
    subdivide(point, count, checkPoint) {
        ...
        for (let i = 1; i < count; i++) {
            const np = new Point(this.x * (1 - (i / count)) + point.x * (i / count), this.y * (1 - (i / count)) + point.y * (i / count), this.z * (1 - (i / count)) + point.z * (i / count));
...
export class HexaSphere {
    constructor(radius, numDivisions, hexSize, scene, viewMode) {
        this.tiles = [];
        this.tileLookup = {};
        this.pathLines = [];
        this.tileLabels = [];
    ...
    generateHexasphere(radius, numDivisions, hexSize, viewMode) {
    ...
    getMountainDensity(lat, lon, radius = 5) {
        let mountainCount = 0;
        let totalSamples = 0;
    ...
    addTileLabel(tileIndex, text, color = 0xffffff, height = 5) {
    ...
    getTilePosition(tileIndex, height) {
        const tile = this.tiles[tileIndex];
        if (!tile)
    ...
    createCurvedLine(startTileIndex, endTileIndex, color = 0x00ffff, segments = 20) {
    ...
    createMeshes() {
        ...
        const supportSetColorAt = typeof instanced.setColorAt === 'function';
    ...
    createPlanetMesh() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const [visualTexture, continentMask] = yield Promise.all([
            ...
            // Invert displacement
            const invertDisplacement = (texture) => {
    ...
    regenerate(radius, numDivisions, hexSize) {
